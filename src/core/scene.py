...
class Scene:
    ...
    def addinto_centred(
        ...
        if line_count > max_lines:  # -2 for the borders
            ...
            skip = self.addinto_centred(
                y_pos, next_lines, delay, pager_delay, color_pair
            )
            ...
            self.addinto_centred(
                y_pos, remaining_lines, delay, pager_delay, color_pair
            )  #
            # recurse for
            # the
            # remaining lines
    ...
    def addinto_all_centred(
        self, text: str, delay: float = 0, pager_delay: float = 2
    ) -> bool:
        """
